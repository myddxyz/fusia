...
# Configuration
class Config:
    # 🔑 Chargement des clés API (avec fallback sur clés codées)
    API_KEYS = []
    ...
    # Si aucune variable d'environnement, utiliser les clés codées
    if not API_KEYS:
        logger.warning("⚠️ Aucune variable d'environnement trouvée, utilisation des clés par défaut")
        API_KEYS = [
    ...
    # Nettoyer les clés (enlever espaces, guillemets, etc.)
    API_KEYS = [key.strip().strip("'").strip('"') for key in API_KEYS if key and key.strip()]
...
if not Config.API_KEYS:
    ...
    logger.error("  export MISTRAL_KEY_1='votre_cle_1'")
    logger.error("  export MISTRAL_KEY_2='votre_cle_2'")
    logger.error("  export MISTRAL_KEY_3='votre_cle_3'")
    logger.error("=" * 70)
...
class MathiaExplorer:
    ...
    def call_mistral_with_retry(self, prompt):
        ...
        try:
            ...
            raise RuntimeError("Module mistralai manquant. Installez-le avec: pip install mistralai")
...
# Template HTML (identique à avant)
...
<body>
    ...
    <script>
        ...
        function initializeApp() {
            console.log('🚀 Mathia Explorer v5.1 - PRODUCTION (Fixed)');
            loadTheme();
            loadLanguage();
...
if __name__ == '__main__':
    print("=" * 70)
    print("🔢 MATHIA V5.1 - PRODUCTION (FIXED)")
    print("=" * 70)
    ...
    try:
        ...
        print("   ✅ Clés API codées en fallback")
        ...
        if any(os.environ.get(f'MISTRAL_KEY_{i}') for i in range(1, 4)):
            print("   ✅ Clés depuis variables d'environnement")
        else:
            print("   ⚠️  Utilisation des clés par défaut (codées en dur)")
        ...
        print("\n💡 Solution:")
        print("   export MISTRAL_KEY_1='votre_cle_1'")
        print("   export MISTRAL_KEY_2='votre_cle_2'")
        print("   export MISTRAL_KEY_3='votre_cle_3'")
